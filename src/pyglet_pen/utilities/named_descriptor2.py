...
class NamedDescriptorMeta(type):
    def __new__(cls, name, bases, cls_dict):
        ...
        for base in bases:
            if base_name_containers:=getattr(base, "__name_containers__"):
        ...
        cls_dict["__name_containers__"] = cls_name_containers
        cls_dict.update(cls_containers)
        return super().__new__(cls, name, bases, cls_dict)
    

class NamedAttributeContainer(metaclass=NamedDescriptorMeta):
    ...
    def attribute_name_containers(cls) -> List[str]:
        return cls.__name_containers__
